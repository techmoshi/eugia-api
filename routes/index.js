...
/**
 * Creating the contact based on category 
 * Join us 
 * contact us
 */
 router.post('/create_contact', router.oauth.authenticate(), function (req, res) {
  console.log(JSON.stringify(router.oauth.authenticate()))
  let validation = Joi.object().keys({
  ...
  Functioncontroller.createContact(req.body).then(blogs =>res.json(blogs) ).catch(error=> { res.json((error)); }   )
  //res.json("Secret");
});

/**
 * update the contact based on category 
 * Join us 
 * contact us
 */
 router.put('/update_contact',upload.single('image'), router.oauth.authenticate(), function (req, res) {
  console.log(JSON.stringify(router.oauth.authenticate()))
 
  console.log(req.body.id)
  Functioncontroller.updateContact(req).then(blogs =>{
    res.status(200).json(blogs) 
  }).catch(error=> { res.json((error)); }   )
  //res.json("Secret");
});
...
/**
 * Get List based on contact category 
 * //etc. example.com/user/000000?sex=female
 * query = {sex:"female"}
 * params = {id:"000000"}
 * 
 */
 router.get('/get_contact', router.oauth.authenticate(), function (req, res) {
  console.log(JSON.stringify(router.oauth.authenticate()))
 
  console.log("Req Data  : ",req.query.category)
  Functioncontroller.getContact(req.query.category).then(blogs =>res.json(blogs) ).catch(error=> { res.json((error)); }   )
